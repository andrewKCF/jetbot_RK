...
def on_cmd_key(msg):
	...
	if (mode=='manual'):
		if (msg.data==65):
			rospy.loginfo("(key)forward")# to print on  terminal
			set_speed(motor_left_ID,0)
			set_speed(motor_right_ID,0)
		elif(msg.data==66):
			rospy.loginfo("(key)backward")# to print on  terminal
			set_speed(motor_left_ID,0.5)
			set_speed(motor_right_ID,0.5)
		elif(msg.data==67):
			rospy.loginfo("(key)right_turn")# to print on  terminal
			set_speed(motor_left_ID,-0.1)
			set_speed(motor_right_ID,-0.2)
		elif(msg.data==68):
			rospy.loginfo("(key)left_turn")# to print on  terminal
			set_speed(motor_left_ID,0.3)
			set_speed(motor_right_ID,-0.3)
		else:
			rospy.loginfo("(key)stop")
			set_speed(motor_left_ID,0)
			set_speed(motor_right_ID,0)
